 ...
 */
window.PrimefacesDialog = window.PrimefacesDialog || (function(globalContext) {
    ...
    function forEachDialog(callback, ctx) {
        forEachProperty(globalContext, function(value, prop, object) {
            if (isDialog(value)) {
    ...
    globalContext.addEventListener('load', function() {
        forEachDialog(function(dialog, name) {
            dialog.cfg.onShow = binding('show', name);
            dialog.cfg.onHide = binding('hide', name);
        });
    });
